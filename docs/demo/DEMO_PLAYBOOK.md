...
> **Total Capabilities: 38** | **Estimated Demo Time: ~30 min**
...
- **Expected:**
  ...
  - [  ] Sources shown below chat message (guideline section codes — only for substantive queries)
...
### TEST 23 - Saved Intelligence Page (Grouped Tiles)
- **Action:** Navigate to `/saved` via sidebar
- **Expected:**
  - [  ] Items grouped by category: Risk Analysis, Memos, Decisions, Conversations, Geospatial, Analytics
  - [  ] Each group has collapse/expand toggle with item count
  - [  ] Cover tile design per item: type badge, title, timestamp, content preview
  - [  ] **Tag chips** on each tile: policy number, claim number, policyholder, industry, underwriter, source
  - [  ] **Tag filter bar** at top — click any tag to filter across all groups
...
## Phase 9: Analytics Playground (5 tests)
...
- **Expected:**
  ...
  - [  ] Empty results pane — plain grid ready for queries
...
- **Expected:**
  ...
  - [  ] **Dynamic grid title:** "Claim Amount, Claim Count by Industry" (auto-generated from selection)
...
- **Action:** Click "Bar" toggle > then "Line" > "Pie" > "Area"
- **Expected:**
  - [  ] Bar chart: Recharts BarChart with coral/purple colored bars
  - [  ] Line chart: Recharts LineChart with multi-line per metric
  - [  ] **Pie chart:** PieChart with color-coded slices and legend
  - [  ] **Area chart:** AreaChart with gradient fill
  - [  ] Grid/Bar/Line/Pie/Area toggle highlights active mode

### TEST 33 - Analytics AI Summary
- **Action:** Click the sparkle icon (AI Summary) on a grid panel header
- **Expected:**
  - [  ] Loading spinner on the button while LLM processes
  - [  ] AI insight panel appears below the grid with narrative analysis
  - [  ] Insight references the actual data in the grid (dimensions, metrics, values)
  - [  ] Dismissible via X button on the insight panel

### TEST 34 - Analytics Save to Saved Intelligence
- **Action:** Click the bookmark icon (Save) on a grid panel header
- **Expected:**
  - [  ] Button changes to filled bookmark with "Saved!" confirmation
  - [  ] Navigate to `/saved` — item appears under "Analytics" category
  - [  ] Tile shows grid title, dimensions, metrics, and timestamp

---

## Phase 10: Session & Guardrails (3 tests)

### TEST 35 - Greeting Citation Suppression
- **Demo Command:** `Hello` or `Hi` or `Good morning`
- **Expected:**
  - [  ] Friendly greeting response from RiskMind
  - [  ] **NO citation codes** below the chat bubble (no 2.2.1, 4.1.1, etc.)
  - [  ] **NO canvas** rendered (stays on welcome screen or current view)
  - [  ] RAG retrieval skipped entirely (saves latency + API cost)
- **Counter-test:** `Why is COMM-2024-016 high risk?` → SHOULD show citations

### TEST 36 - Login/Logout Fresh Chat
- **Action:** Have an active chat session > Logout > Login again
- **Expected:**
  - [  ] Chat window is completely fresh (no previous messages)
  - [  ] Previous session preserved in session history for 2 days
  - [  ] Session history accessible via history panel
  - [  ] Different user login shows only their own sessions

### TEST 37 - Document Session Context
- **Action:** Upload a PDF via paperclip > then ask `Can you summarize that document?`
- **Expected:**
  - [  ] Upload response includes document analysis
  - [  ] Follow-up question references the uploaded document (not generic portfolio)
  - [  ] Session ID links the upload to the chat conversation
  - [  ] Document appears in `UPLOADED DOCUMENTS IN THIS SESSION` context

---

## Phase 11: RBAC & Edge Cases (1 test)

### TEST 38 - User Data Scoping (RBAC)
...
"For ad-hoc exploration, the Analytics Playground lets me pick dimensions, measures, and filters. Toggle between grid, bar, line, pie, and area charts. I can get an AI summary of any grid — click the sparkle icon — and the AI explains the patterns in the data. Save any analysis directly to Saved Intelligence with one click."

**Saved Intelligence (1 min):**
"Everything I save is organized automatically — grouped by type: Risk Analysis, Memos, Decisions, Analytics, Geospatial. Each tile has tag chips: policy number, policyholder, industry, who saved it. I can filter by any tag to find what I need. Export individual items or the whole report as PDF."

**Smart Guardrails (30 sec):**
"Notice when I say 'Hello' — no citations, no irrelevant data. The AI knows when to stay conversational. But the moment I ask about a policy, it pulls the right guidelines and cites them. The system also maintains document context within a session — I can upload an inspection report, then ask follow-up questions about it without repeating what I uploaded."
...
|---|-----------|-----------------|
| 1 | **RAG Pipeline** | ChromaDB indexes guidelines + claims + uploaded docs → cited in substantive responses |
...
| 4 | **Guardrails** | Out-of-scope rejection, greeting detection, hallucination detection, entity redaction |
| 5 | **Conversational Memory** | Follow-up questions resolve entities from chat history + document session context |
...
| 11 | **Smart Citations** | RAG skipped for greetings/off-topic — citations only for data-driven answers |
| 12 | **Saved Intelligence** | Categorized tiles with tag chips, filter bar, PDF export, grouped by analysis type |
| 13 | **Analytics AI** | Per-grid AI summary, 5 viz types (grid/bar/line/pie/area), save to intelligence |
